...
class LinearRegressionCustom:
    def __init__(self, features: int, lr: float, epochs: int):
        self.w = np.random.normal(0, 0.1, [features])
        self.b = 0.0
        self.lr = lr
        self.epochs = epochs

    def calculate_loss(self, y_pred: np.array, y: np.array):
        return np.mean((y_pred - y) ** 2)

    def predict(self, X: np.array):
        # X: (n_samples, n_features), w: (n_features,)
    ...
    def train(self, X: np.array, y: np.array):
        ...
        for i in range(self.epochs):
            print(f"{i}th epoch started")
            y_pred = self.predict(X)
            self.update_weights(X, y, y_pred)
            loss.append(self.calculate_loss(y_pred, y))
...
sklearn_pred_scaled = sklearn_lr.predict(x_test_scaled)
...
sgd_pred_scaled = sgd.predict(x_test_scaled)
sgd_pred = y_transform.inverse_transform(sgd_pred_scaled.reshape(-1, 1))
