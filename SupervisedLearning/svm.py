...
x_train,x_test,y_train,y_test = train_test_split(encoded_df[["Age","Gender_Male","EstimatedSalary"]],encoded_df[["Purchased"]],shuffle=True)
    ...
    def fit(self,x:np.array,y:np.array):
        ...
        for idx,x_i in enumerate(x):
            ...
            if constarin :
                ...
                self.w -= self.lr * (2 * self.cost * self.w- (np.dot(x_i, y[idx])).reshape(-1,1))
