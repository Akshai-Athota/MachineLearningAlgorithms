...
class Node():
    def __init__(self,feature_index : Optional[int]=None,threshold : Optional[float]=None,left:Optional["Node"]=None,
                 right:Optional["Node"]=None,value : Optional[Union[int,float]] = None):
...
class Decision_Tree_Classifier():
    ...
    def __init__(self,max_depth: int = 5,min_samples_split: int =2,min_samples_leaf:int =1,criterion : str ="entropy"):
    ...
    def _best_fit(self,x:np.ndarray,y:np.ndarray):
        ...
        for feature_index in range(n_features):
            ...
            for thres in thresholds:
                ...
                weighted_criterion = ((left_mask).sum()*left_criterion + (right_mask.sum())*right_criterion)/n_samples
    ...
    def _build_tree(self,x:np.ndarray,y:np.ndarray,depth:int=0):
        ...
        if (n_samples < self.min_samples_split or n_features ==1 or n_labels ==1 or
